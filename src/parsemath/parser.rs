...
impl<'a> Parser<'a> {
    ...
    pub fn parse(&mut self) -> Result<Node, ParseError> {
        let ast = self.genereate_ast(OperPrec::DefaultZero);
        match ast {
            Ok(ast) => Ok(ast),
            Err(e) => Err(e)
        }
    ...
    fn genereate_ast(&mut self, oper_prec: OperPrec) -> Result<Node, ParseError> {
    ...
    fn parse_number(&mut self) -> Result<Node, ParseError> {
        ...
        match token {
            Token::Substract => {
                self.get_next_token()?;
                let expr = self.genereate_ast(OperPrec::Negative)?;
            ...
            Token::LeftParen => {
                self.get_next_token()?;
                let expr = self.genereate_ast(OperPrec::DefaultZero)?;
                self.check_paren(Token::RightParen)?;
                if self.current_token == Token::LeftParen {
                    let right = self.genereate_ast(OperPrec::MulDiv)?;
    ...
    fn convert_token_to_node(&mut self, left_expr: Node) -> Result<Node, ParseError> {
        match self.current_token {
            Token::Add => {
                ...
                let right_expr = self.genereate_ast(OperPrec::AddSub)?;
            ...
            Token::Multiply => {
                self.get_next_token()?;
                // Get right-side expression
                let right_expr = self.genereate_ast(OperPrec::MulDiv)?;
            ...
            Token::Caret => {
                self.get_next_token()?;
                // Get right-side expression
                let right_expr = self.genereate_ast(OperPrec::Power)?;
